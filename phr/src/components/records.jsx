        ...
        { // the ? is use as a if statement, here it is making sure if that data is available or not.
          data?.map((item, index) => ( // every column will have its own grid
            <div 
              ...
              onClick={() => navigate('/view-doctor-details', { state: { doctor_id: item?.doctor_id } })} // this is how we can pass data to another page using react router dom.
            >
...
export function Notes({nic, doctor_id}){
  ...
  return(
    <div className="h-full space-y-3 overflow-y-auto pr-1 md:pr-2">
      {visitNotes?.map((item, index) => (
        <div
          key={item?.note_id ?? index}                                                                                           // smooth transition between colors
          className="flex items-center justify-between rounded-2xl border border-[#7A7979] bg-white px-4 py-4 shadow-sm transition-colors hover:bg-slate-100 [scrollbar-gutter:stable]"
          onClick={(e) => {
            e.stopPropagation();
            navigate(`/view-note`, {state: {note_id: item?.note_id}} )
          }}
        >
    ...
    <div className="h-full space-y-3 overflow-y-auto pr-1 md:pr-2 [scrollbar-gutter:stable]">
      {LabData.map((item, index) => (
        <div
          ...
          onClick={(e) => {
            e.stopPropagation();
            navigate(`/view-report`, {state: {report_id: item?.report_id}} )
          }}
        >
          <div className="flex items-center gap-4">
